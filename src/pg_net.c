...
static int pgnet_exchange_bootstrap(int left_fd, int right_fd, struct pg *pg) {
    ...
    pack_boot_for_right(pg, &local_gid, port_attr.lid, psn_right, &my_right); // exports qp_right
    ...
    if (pg->rank == 0) {
        fprintf(stderr, "[boot] (rank0) exch with right (send-first): send my R=%u psn=%u, then recv peer_left\n", my_right.qpn, my_right.psn);
        ...
        fprintf(stderr, "[boot] (rank0) exch with left (recv-first): recv peer_right, then send my L=%u psn=%u\n", my_left.qpn, my_left.psn);
        ...
        fprintf(stderr, "[boot] exch with left (recv-first): recv peer_right, then send my L=%u psn=%u\n", my_left.qpn, my_left.psn);
        fprintf(stderr, "[boot] (rank%d) exch with left:  sending my L=%u, expecting peer_right\n",
                pg->rank, pg->qp_left->qp_num);
        if (pgnet_exchange_qp(left_fd, &my_left, &pg->left_qp, 0) != 0) {
            fprintf(stderr, "Bootstrap exchange with left neighbor failed\n");
        ...
        fprintf(stderr, "[boot] exch with right (send-first): send my R=%u psn=%u, then recv peer_left\n", my_right.qpn, my_right.psn);
        fprintf(stderr, "[boot] (rank%d) exch with right: sending my R=%u, expecting peer_left\n",
                pg->rank, pg->qp_right->qp_num);
        if (pgnet_exchange_qp(right_fd, &my_right, &pg->right_qp, 1) != 0) {
            fprintf(stderr, "Bootstrap exchange with right neighbor failed\n");
    ...
    // Check summary for cross-rank matches
    fprintf(stderr, "[check] right_blob.qpn (got=%u) should equal next.rank qp_left\n",  pg->right_qp.qpn);
    fprintf(stderr, "[check] left_blob.qpn  (got=%u) should equal prev.rank qp_right\n", pg->left_qp.qpn);
