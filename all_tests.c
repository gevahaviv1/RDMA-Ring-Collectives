...
int ibv_poll_cq(struct ibv_cq *cq, int num_entries, struct ibv_wc *wc) {
    if (!ibv_poll_cq_impl) return 0;
    return ibv_poll_cq_impl(cq, num_entries, wc);
}

static int ibv_poll_cq_queue(struct ibv_cq *cq, int num_entries, struct ibv_wc *wc) {
    int n = 0;
...
static void test_bootstrap(void) {
    ...
    if (pid == 0) {
        ...
        if (bootstrap_ring(hosts, 2, 0, base_port, 5000, &fd_left, &fd_right) != 0)
            return;
        if (write(fd_right, "a", 1) != 1) return;
        char ch;
        if (read(fd_left, &ch, 1) != 1) return;
        assert(ch == 'b');
        struct qp_boot my = {
            .qpn = 10,
            .psn = 20,
...
static void test_pg_close(void) {
    destroy_qp_calls = destroy_cq_calls = dereg_mr_calls = dealloc_pd_calls = close_dev_calls = 0;
    pg_handle *h = calloc(1, sizeof(pg_handle));
    assert(h);
    struct ibv_qp qp; struct ibv_cq cq; struct ibv_mr mr1; struct ibv_mr mr_ctrl; struct ibv_pd pd; struct ibv_context ctx;
    h->qps[0] = &qp;
    h->cq = &cq;
    h->data_mrs[0] = &mr1;
    h->ctrl_mr = &mr_ctrl;
...
static void re_on_chunk(int chunk_id, void *ptr, size_t bytes) {
    ...
    assert(memcmp(ptr, re_remote_buf + (size_t)chunk_id * re_chunk_size, bytes) == 0);
}
...
static void test_read_engine(void) {
    ...
    for (size_t i = 0; i < total_bytes; ++i) remote[i] = (char)i;
    ...
    int rc = rdma_read_engine((uintptr_t)remote, 0, (uintptr_t)local_buf, 0,
                              total_bytes, chunk_bytes, inflight_limit, &qp, re_on_chunk);
    assert(rc == 0);
...
static void rtscts_on_chunk(int chunk_id, void *ptr, size_t bytes) {
    ...
    assert(memcmp(ptr, rtscts_remote_ptr + (size_t)abs_chunk * rtscts_chunk_bytes, bytes) == 0);
}
static void test_rtscts(void) {
    ...
    for (size_t i = 0; i < total_bytes; ++i) right_buf[i] = (char)i;
    ...
    for (int chunk = 0; chunk < num_chunks; ++chunk) {
        ...
        int rc = rdma_read_engine((uintptr_t)(right_buf + (size_t)chunk * chunk_bytes), 0,
                                  ...
                                  bytes, bytes, 1, &qp, rtscts_on_chunk);
        assert(rc == 0);
